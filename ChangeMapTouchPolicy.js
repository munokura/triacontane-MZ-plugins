...
@plugindesc Map Touch Specification Change Plugin
@author Triacontane
@license MIT License
...
You can change the movement behavior when touching the map from four options.
Please set the "Default Specification Level" parameter.
1: Disabled (Touching the map does not move the player)
2: Simple (Touching the map moves the player one square)
(Does not move to a specified location. Continue touching to continue moving)
3: Normal (Touching the map moves the player to that location)
(This is the original behavior)
4: Excessive (Touching the map once will cause the player to track the
pointer)
(Touching again will stop movement)

If you want to change the specification level later, specify the control
character \v[n] in the parameter.
...
@text Specification level initial value
@desc Initial Map Touch specification level
@type select
@default 3
@option 1: Disabled
@value 1
...
@text Forced Dash Disabled
@desc Disables forced dash when moving by touching the map, and dashes only while holding down the Shift key. Excludes the mobile version.
@type boolean
@default false
*/
