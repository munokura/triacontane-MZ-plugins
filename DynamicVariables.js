...
@plugindesc Dynamic Variables Plugin
@author Triacontane
@license MIT License
...
When referencing a variable or switch within the specified range
Returns the result of evaluating the "variable name" and "switch name" as a
script.
In addition to control characters, the following local variables can be used
in scripts.

id # Target switch, variable ID
value # Target switch, variable ID's original value

When referencing a switch in the appearance conditions of an event page, the
following variables can be used.
e # Reference to the event object
d # Reference to the event data

When referencing a switch in an enemy character's behavior pattern, the
following variables can be used.
e # Reference to the enemy character object
d # Reference to the enemy character data

The following variables can be used regardless of the situation.
v # Reference to the game variable object
s # Reference to the game switch object

Dynamic variables are valid in all places where variables and switches are
referenced (see below).

Examples of referencing variables and switches
1. Event page determination process
2. Enemy character action determination process
3. Conditional branching
4. Event command operands
5. References by other plugins

You can also reference dynamic variables and switches within a script.
However, attempting to reference a variable (switch) with the same number will
result in a circular reference and an error.

Originally, the event page determination process would only be executed when
any switch or variable is changed.
This plugin changes the page determination process to run every frame if you
set even one dynamic switch or dynamic variable as a page appearance
condition.
This process slightly degrades performance, so if you don't want it to run
every frame even when using a dynamic switch, please specify the following in
the notes field:
<NoRefresh>
...
@text Dynamic switch start position
@desc The starting position number of the dynamic switch.
@type switch
@default 0

@param DynamicSwitchEnd
@text Dynamic switch end position
@desc End position number of the dynamic switch.
@type switch
@default 0

@param DynamicVariableStart
@text Dynamic variable start position
@desc The starting position number of the dynamic variable.
@type variable
@default 0

@param DynamicVariableEnd
@text Dynamic variable end position
@desc The ending position number of the dynamic variable.
@type variable
@default 0

@param ValidException
@text Exception Handling
@desc Exception handling will be performed when the script is executed. Invalid scripts will no longer be forcibly terminated, but the execution speed will be slightly reduced.
@type boolean
@default false
*/
