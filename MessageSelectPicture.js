...
@plugindesc Picture display plugin for options
@author Triacontane
@license MIT License
...
Use the "Show Options" event command to display the corresponding picture when
you hover over an option.

Prepare the picture with an opacity of 0, then execute the plugin command to
specify the association.

The plugin will not hide the picture after you select an option, so feel free
to use the event command to manipulate it.
...
@command SET_UP
@text set up
@desc Sets up the information for the choice picture. The setting will be cleared once the choice is displayed.
@arg pictureIdList
@text Picture Number List
@desc Defines the picture number that corresponds to the option.
@type number[]
@default []
*/
