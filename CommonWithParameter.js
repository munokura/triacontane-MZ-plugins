...
@plugindesc Common Call Plugin with Arguments
@author Triacontane
@license MIT License
...
You can call common events with arguments.
Register the variable number to which arguments are passed in the plugin
parameter,
and call the common event by specifying the parameter from the dedicated
plugin command.

You can also define common arguments for all common events.
In that case, specify 0 as the common event ID and
define that data at the bottom of the list.

You can also call common events based on their name.
Calling common events by name uses a partial name match search.
...
@param likeSearch
@text Partial name match search
@desc When specifying a common event to call by name, a partial match search will be performed.
@type boolean
@default true

@param arguments
@text Argument List
@desc Registers argument information for a common event.
@type struct<CommonArguments>[]
@default []

@command CALL
@text Common Event Call
@desc Calls a common event with special parameters.
@arg id
@text Common Event ID
@desc This is the common event ID of the event you want to call. If you want to call it by name, enter the name in the Text tab.
@type common_event
@default 0
@arg parameters
@text Parameters
@desc The value specified here will be stored in the variable specified in the argument information of the plugin parameter.
...
@param label
@text label
@desc This is an identification label. It has no particular purpose, so give it an easy-to-identify name.
@type string

@param id
@text Common Event ID
@desc This is the ID of the common event for which argument information is to be registered. If you specify 0, the variable will be shared by all common events.
@type common_event
@default 0

@param variables
@text Variable in which the argument is stored
@desc A list of variable numbers whose values will be stored when the function is called.
@type variable[]
@default []
*/
