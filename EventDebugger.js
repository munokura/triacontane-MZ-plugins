...
You can pause event execution at any point and execute it line by line (step
execution).

There are three ways to start it:

1. Press the specified key (default: F7). (*1)
2. Execute the plugin command [Breakpoint] (conditions can be specified).
3. Execute an event test.
*1 This method does not allow step execution of parallel events.

While execution is paused, the following operations are possible:

1. Step execution (execute events one command at a time)
2. Check the list of running events
3. Check the parameters of executed events
4. Open the debug screen (F9) and manipulate switches and variables (*2)
5. Execute arbitrary JavaScript from the console
*2 Only available on the map screen.

The following types of step execution are available, and you can specify the
corresponding function keys.

Step In: Executes one line at a time. Also steps through calls to common
events.
Step Over: Executes one line at a time. Executes all common event calls at
once.
Resume: Ends step execution and returns to normal event execution.

It also outputs the time it took to execute each event command.

You can also constantly monitor the evaluation results of variables and
scripts.
If a target to be monitored is registered, a window will appear in the upper
right.
If a number is specified, the variable value will be displayed, and if a
string is specified, the script evaluation result will be displayed.
You can register a target to be monitored by pressing the specified function
key or by running the following script
from the Developer Tools.

DebugManager.watchVariable(5); # Constantly monitor the value of variable [5].

This plugin only works in test mode and has no effect on the retail version.
...
@param stepStart
@text Step Start
@desc This is a function key to start step execution from the next executed event command.
@type select
@default F7
@option none
@option F1
...
@param stepIn
@text Step Into
@desc This is a function key for stepping in (executing one command) during step execution.
@type select
@default F11
@option none
@option F1
...
@param stepOver
@text Stepover
@desc This is a function key to step over (execute one command) during step execution. It skips common events.
@type select
@default F10
@option none
@option F1
...
@param continue
@text Continue
@desc Function key to interrupt and continue step execution.
@type select
@default F6
@option none
@option F1
...
@desc Toggles the display state of the debug window. You can also toggle it with the Shift key.
@type select
@default none
@option none
@option F1
...
@param watchVariable
@text Variable Watch
@desc A dialog will appear where you can enter the game variables or scripts you want to monitor.
@type select
@default F1
@option none
@option F1
...
@param maxWatchNum
@text Maximum number of monitors
@desc The maximum number of variables to watch. Specifying a very large value may slow performance.
@type number
@default 3

@param eventTest
@text Event Test
@desc When you perform an event test, step execution will occur automatically.
@type boolean
@default true

@param suppressFunc
@text Function Key Suppression
@desc When you press a function key, it suppresses the default and other plugins from working.
@type boolean
@default false

@param okHandler
@text OK operation
@desc Sets the function key behavior when the Enter button is pressed during step execution.
@type select
@default F11
@option none
@option F1
...
@param cancelHandler
@text Cancel Action
@desc Sets the function key behavior when the Cancel button is pressed during step execution.
@type select
@default F6
@option none
@option F1
...
@text Press Ctrl simultaneously
@desc Whether or not you need to press the Ctrl key simultaneously when using each function. Use this when there is a conflict with the target key of another plugin.
@type boolean
@default false

@param simultaneousAlt
@text Press Alt simultaneously
@desc Whether or not the Alt key must be pressed simultaneously when using each function. Use this when there is a conflict with the target key of another plugin.
@type boolean
@default false

@param scriptDebug
@text Script Debugging
@desc Sets the behavior when an error occurs in the "script" of the event command. 0: Error (normal) 1: Step execution 2: Ignore
@type select
@default 0
@option Error (as usual)
@value 0
@option Step execution
@value 1
@option ignore
@value 2

@param disableDebugCtrlKey
@text Disable with CTRL
@desc While you hold down the CTRL (option on Mac) key, step execution will not occur even if the step execution conditions are met.
@type boolean
@default true

@param showDevTools
@text Developer Tools View
@desc When enabled, the developer tools will be displayed when you start debugging.
@type boolean
@default true

@param breakSwitchId
@text Break Switch Number
@desc Debug execution will begin when the specified switch is turned ON.
@type switch
@default 0

@command BREAK_POINT
@text Breakpoints
@desc Set a breakpoint, optionally specifying a condition.
@arg script
@text script
@desc Used to specify a condition that, when met, activates the breakpoint.
*/
