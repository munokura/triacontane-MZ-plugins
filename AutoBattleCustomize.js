...
@plugindesc Automatic Battle Adjustment Plugin
@author Triacontane
@license MIT License
...
Adjusts the behavioral criteria for auto-battle.
This is not intended to provide an advanced AI.
The current function is to only consider skills with MP effects.

The default auto-battle specifications are as follows:

- Only uses skills with HP effects (excluding skills with usage effects or MP
effects only).

- For damage and recovery, uses the skill with the highest effect ratio to
maximum HP.

- Does not take skill cost (MP, TP) into consideration.

- Does not use items.

- Actions are determined only at the start of the turn.
...
@param mpEffectRate
@text MP effect action adoption rate
@desc This is the degree to which skills that have an effect on MP are added to the list of possible uses. At a level of 100, the skill is considered to have the same effect as HP.
@type number
@default 0
@min 0
@max 100
...
@text Updated just before the start
@desc Auto-combat actions and targets will be updated again just before the action.
@type boolean
@default false
*/
