...
@param CommandId
@text Command ID
@desc Subcommand identification number. Commands are grouped together using this number and name. Normally, it is fine to set all to 0.
@default 0

@param Name
@text Command Name
@desc Name displayed as a subcommand
@default サブコマンド1

@param ParentName
@text Parent name
@desc The parent name of the subcommand displayed in the main command. Subcommands with the same parent name will be grouped together.
@default 親コマンド1

@param HiddenSwitchId
@text Hidden Switch Number
@desc Switch ID that hides commands when ON
@type switch
@default 0

@param DisableSwitchId
@text Disabled switch number
@desc Switch ID that disables commands when ON
@type switch
@default 0

@param Script
@text script
@desc Script to be executed when the command is determined
@type combo
@option this.commandItem(); // Open the item screen
@option this.commandSave(); // Open the save screen
@option SceneManager.push(Scene_Load); // Open the loading screen
@option this.commandOptions(); // Open the options screen
@option this.commandGlossary(1); // Call the glossary (when using the glossary plugin)
@option $gameSwitches.setValue(1, true); // Turn on switch [1]
@option $gameVariables.setValue(1, 10); // Assign [10] to variable [1]
@option $gameTemp.reserveCommonEvent(1); // After returning to the map, call the common event [1].

@param ReturnMap
@text Return to map?
@desc After the script runs, return to the map.
@type boolean
@default false

@param MapId
@text Destination map ID
@desc Map ID to move to when the command is confirmed
@default 0

@param SelectMember
@text Member selection available
@desc Whether to select target members before executing the command
@type boolean
@default false
*/
