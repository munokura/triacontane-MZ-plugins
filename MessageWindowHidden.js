...
@plugindesc Message Window Temporarily Clear Plugin
@author Triacontane
@license MIT License
...
Pressing the specified button while the message window is displayed will
dismiss the message window. Pressing it again will return it.

You can specify a picture whose opacity will be set to [0] when the window is
hidden.
Use this if you are using a specific picture as the background.
When the window is redisplayed, the opacity will be restored to the opacity it
had before it was hidden.

As of version 2.0.0, the method for specifying parameters has changed.
If you were using a previous version, please reset your settings.
...
@param triggerButton
@text Button Name
@desc This is a button to clear the window. (Multiple buttons can be registered.) If you have added a button that can be input using a plugin, you can input it directly.
@type combo[]
@default ["右クリック"]
@option 右クリック
@option shift
...
@param triggerSwitch
@text Trigger switch
@desc The window will be cleared in conjunction with the specified switch. Please manage ON/OFF appropriately using parallel processing etc.
@type switch
@default 0

@param syncSwitch
@text Trigger Switch Synchronization
@desc When enabled, the trigger switch will synchronize with the window visibility state, even if it is manually dismissed.
@type boolean
@default false

@param linkPictureNumbers
@text Linked Picture Number
@desc This is the number of the picture whose opacity will be set to [0] when the window is cleared.
@type number[]
@default []

@param linkShowPictureNumbers
@text Interlocking picture number
@desc This is the number of the picture whose opacity will be set to [255] when the window is cleared.
@type number[]
@default []

@param disableLinkSwitchId
@text Linked picture disable switch
@desc When the switch with the specified number is ON, picture linking will be disabled.
@type switch
@default 0

@param disableSwitchId
@text Disable Switch
@desc When the switch with the specified number is ON, the entire plug-in function will be disabled.
@type switch
@default 0

@param disableInBattle
@text Disabled during battle
@desc If true, disables plugin functionality during battle.
@type boolean
@default false

@param disableInChoice
@text Invalid while options are displayed
@desc The window cannot be hidden while options are being displayed.
@type boolean
@default true

@param restoreByDecision
@text Return with the decision action
@desc When clearing a message, you can redisplay the hidden message window by pressing the confirm button.
@type boolean
@default false
*/
