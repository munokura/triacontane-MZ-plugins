...
// ----------------------------------------------------------------------------
// Version
// 1.9.4 2022/11/26 ヘルプに誤った記載があったため修正
...
@plugindesc Bust-up display plugin
@author Triacontane
@license MIT License
...
Displays a bust-up image for each actor on the status screen.
You can freely adjust the display position, with the feet as the origin.

The same bust-up image can also be displayed on the equipment screen and skill
screen.
However, since there is not enough space on the default screen size, please
change the screen size as necessary.

The same bust-up image can also be displayed on the main menu screen, but
only the "first actor" will be displayed.
This is mainly used when there is only one actor.

Specify the following in the actor's memo field.

<SBIImage:file> # /img/pictures/file.png will be displayed.
<SBIImage:file> # Same as above
<SBIRect:0,0,100,100> # The image will be cropped and displayed in the
specified rectangle (X coordinates, Y coordinates, width, height). (Comma
separated)
<SBIImageZoomX:100> # Sets the horizontal zoom ratio of the image.
<SBIImageScaleX:100> # Same as above
<SBIImageScaleY:100> # Sets the vertical scale of the image.
<SBIImageScaleY:100> # Same as above

You can also display multiple additional variations in the memo field below.
<SBIAddImage1:file2> # Displays /img/pictures/file2.png.
<SBIAddImage1:file2> # Same as above
<SBIAddCondition1:\v[1] === 3> # The additional image is displayed only when
variable [1] is equal to [3].
<SBIAddCond1:\v[1] === 3> # Same as above
<SBIAddPosX1:30> # Sets the X coordinate of the additional image to [30].
<SBIAddPosX1:30> # Same as above
<SBIAddPosY1:30> # Sets the Y coordinate of the additional image to [30].
<SBIAddPosY1:30> # Same as above
<SBI Additional Coordinate Scale X1:100> # Sets the horizontal scale of the
additional image.
<SBIAddPosScaleX1:100> # Same as above
<SBI Additional Coordinate Scale Y1:100> # Sets the vertical scale of the
additional image.
<SBIAddPosScaleY1:100> # Same as above

To display multiple additional images, change the last number to [2] or
higher.

The specified coordinates are relative to the base of the base image.

The inequality sign ">" cannot be used in the calculation formula.
You can use "<".

You can add images for each equipment, state, and job.
Specify each in the database memo field as follows:
<SBIImage:item> # /img/pictures/item.png will be displayed.
<SBIImage:item> # /img/pictures/item.png will be displayed.
<SBIPosX:30> # Sets the X coordinate of the equipment image to [30].
<SBI Coordinate X:30> # Sets the X coordinate of the equipment image to [30].
<SBIPosY:30> # Sets the Y coordinate of the equipment image to [30].
<SBI Coordinate Y:30> # Sets the Y coordinate of the equipment image to [30].
<SBIPosZ:3> # Sets the Z coordinate of the equipment image to [3].
<SBI Coordinate Z:3> # Sets the Z coordinate of the equipment image to [3].
<SBI Rect:0,0,100,100> # Displays the image cropped to the specified rectangle
(X coordinate, Y coordinate, width, height). (Comma separated)
<SBIScaleX:100> # Sets the horizontal scale of the equipment image.
<SBI Scale X: 100> # Same as above
<SBI Scale Y: 100> # Sets the vertical scale of the equipment image.
<SBI Scale Y: 100> # Same as above

The specified coordinates are relative to the base image's feet.

The larger the Z coordinate, the closer the image will be displayed. If not
specified, it will default to [1].

The Z coordinate of the actor image is fixed at [0].

You can also change images and videos by executing plugin commands.
Use this if you want to replace them as the story progresses.
...
@desc Replace the actor's bust image.
@arg actorId
@text Actor ID
@desc The actor ID to be replaced.
@type actor
@default 1
@arg fileName
@text File name
@desc The image file to replace.
@type file
@dir img/pictures
*/
