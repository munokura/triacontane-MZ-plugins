...
@plugindesc Parameter Conversion Plugin
@author Triacontane
@license MIT License
...
Dynamic conversion of the following eight main parameters according to
specified rules.
You can reference the value of other parameters, or multiply or add values.
・Max HP (0)
・Max MP (1)
・Attack Power (2)
・Defense Power (3)
...
Please fill in the memo field of the database with characteristics according
to each function.

1. Convert one parameter to another.
You can create equipment in which Max HP and Max MP are temporarily swapped,
or
states in which Magic Power is converted to Attack Power.

<PT0:1> # Max HP value is overwritten with Max MP.
<PT1:0> # Max MP value is overwritten with Max HP.
*See the above description for numbers. (Same applies hereafter.)

2. Multiply a parameter by an arbitrary value.
You can use the control character \v[n] and JavaScript expressions for the
multiplier.
<PTRate0:\v[1]+50> # Maximum HP is multiplied by "variable[1] value + 50%."

You can also reference the target battler's information by entering "battler"
in the formula.
<PTRate2:100 + battler.tp> # Attack power varies based on current TP.
(100 TP doubles attack power)

3. Add a value to the parameter.
<PTAdd4:50> # Magic power is increased by 50.
The control character \v[n] and JavaScript expressions can be used for the
additional value.

The conversion only applies to base parameters, not to equipment or buffs.

This script retrieves the actor's parameters before conversion.
