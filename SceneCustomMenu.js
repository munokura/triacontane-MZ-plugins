...
@param Id
@text Scene Identifier
@desc This is the identifier used to call the scene. Please specify a string that does not overlap with other identifiers.
@type string
@default Scene_Test

@param UseHelp
@text Use the Help window
@desc If enabled, displays the help window.
@type select
@default 1
@option Not used
@value 0
@option Display at the bottom of the screen (MZ default)
@value 1
@option Displayed at the top of the screen
@value 2
...
@desc This is a common event that is executed continuously while the scene is displayed. Use with caution as it may decrease performance.
@type common_event
@default 0

@param ActorChangeEvent
@text Actor Change Event
@desc This event occurs the moment the actor is changed. This event does not change the window focus.
@type struct<Event>
...
@text Using the page buttons
@desc If enabled, the page buttons will be displayed.
@type boolean
@default false

@param SnapNoFilter
@text Disable background blur
@desc If specified, background snap blur will not be applied.
@type boolean
@default false
*/
...
@param originX
@text X-axis origin
@desc The origin that determines the window coordinates. If you specify this, please also specify the width.
@type select
@default 0
@option left
@value 0
...
@param Rotation
@text Rotation angle
@desc The angle of the window. It is specified in degrees (0-360). There is a restriction that the content filter will not be effective.
@type number
@default 0
...
@type combo
@option item.meta['value']; // The memo field contains the description <value>
...
@param SortScript
@text Sort Script
@desc Sorts the items in the list. Variable [a] and variable [b] are references to each element for comparison.
@type combo
@option a.id - b.id; // ID order
@option a.name.localeCompare(b.name); // By name
@option this.intMeta(a,'order') - this.intMeta(b,'order'); // Order of memo field
@option a.price - b.price; // Price order
@option a.params[0] - b.params[0]; // HP order
@option a.params[1] - b.params[1]; // MP order
@option a.params[2] - b.params[2]; // Attack Power
@option a.params[3] - b.params[3]; // Defense power order
@option a.params[4] - b.params[4]; // Magic power order
@option a.params[5] - b.params[5]; // Magic defense order
@option a.params[6] - b.params[6]; // Agility order
@option a.params[7] - b.params[7]; // luck
@parent DataScript
...
@option this.drawItemName(item, r.x, r.y, r.width); // Item and skill names (including icons)
@option this.drawText($gameParty.numItems(item), r.x, r.y, r.width, 'right'); // Number of items owned
...
@option this.changeTextColor(ColorManager.textColor(1)); // Change text color (only valid for drawText)
@option this.drawText(this.findWindowItem('window1').name, r.x, r.y, r.width); // Name of the item selected in another window
@option this.drawNotePicture('noteValue', r.x, r.y, 'left', 'center', 1.0, 1.0); // Draws a picture of the specified memo field
@option this.placeActorName(item, r.x, r.y); // Actor Name (for battle)
@option this.placeStateIcon(item, r.x, r.y); // State Icon (for battle)
@option this.placeGauge(item, 'hp', r.x, r.y); // HP gauge (for battle)
@option this.placeBasicGauges(item, r.x, r.y); // Gauge set (for battle)
@option this.drawNoteText('noteValue', r.x, r.y); // Draw the contents of the specified memo field
@option this.drawNoteText('noteValue', r.x, r.y, 'right'); // Right-justified drawing of memo field contents
@option this.drawSavefileInfo(item, r.x, r.y, r.width); // Draw the contents of the save file
@parent DataScript

@param ItemDrawMultiLineScript
@text Drawing scripts (multiple)
@desc This is a script that draws items. Each element can be referenced from the variable [item]. Use this when you want to enter a multi-line script.
...
@desc Help text that is displayed regardless of the selected item.
@type multiline_string
...
@desc This event occurs when a cancellation occurs.
@type struct<Event>
@default {}
...
@param FontFace
@text font
@desc Changes the window font. The function to load font files is not provided, so please prepare them separately.
...
@desc When enabled, you will be able to change actors with PageUp and PageDown.
@type boolean
@default false
...
@param DarkNoFocus
@text Darkens when out of focus
@desc When enabled, the window contents will be dimmed when the window is not in focus.
@type boolean
@default false
...
@text Bring cursor to the front
@desc When enabled, the window cursor will appear over the item.
@type boolean
@default false

@param noItemBackground
@text Do not display item background
@desc When enabled, the black background of the item will not be displayed.
@type boolean
@default false

@param noFrame
@text Do not display frame
@desc When enabled, window borders will not be displayed.
@type boolean
@default false
...
@text All select switch ID
@desc When the specified switch is ON, the cursor will be in full selection state.
@type switch
@default 0

@param cursorFixedSwitchId
@text Fixed selection switch ID
@desc When the specified switch is ON, the cursor selection is fixed.
@type switch
@default 0
*/
...
@param HelpText
@text Help Text
@desc If the help window is visible, the help text will be displayed.
@type multiline_string

@param DecisionEvent
@text Decision Event
@desc An event that occurs the moment this item is decided. If specified, it takes precedence over the common decision event.
@type struct<Event>
...
@option this.executeSave(v(1)); // Save execution
@option this.executeLoad(v(1)); // Load Execution
