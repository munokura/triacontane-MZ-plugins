...
Play multiple BGS in parallel.
By changing the BGS playback line using the plugin command, you can play a new
BGS while the original BGS continues playing.
You can individually adjust the volume, pitch, and left/right balance of each
...
simultaneously may affect performance.

Note that the auto-play BGS specified in the map will have a fixed line of
[1].
...
@command CHANGE_LINE
@text Line change
@desc Changes the playing line of the current BGS. BGS with different playing lines can be played in parallel.
@arg line
@text Line Number
@desc The line number of the BGS to change.
@type number
@default 1

@command ALL_STOP
@text Full stop
@desc Stops all background music currently playing.

@command FADEOUT_FOR_SE
@text Fade out when SE is played
@desc The background music will automatically fade out when the sound effect is played.
@arg type
@text Fade-out type
@desc How to fade out.
@type select
@default 1
@option 0: Play the SE normally without fading out.
@value 0
@option 1: The fade-out will start simultaneously with the SE playing.
@value 1
@option 2: The SE will play after the fade-out is complete.
@value 2
@arg fadeTime
@text Fade Time
@desc The time (in frames) it takes for the automatic fade out.
@type number
@default 60
*/
